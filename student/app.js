App({
        ...
        // this.globalData.code=res.code
          wx.getUserInfo({
            success: res => {
              ...
              // 由于 getUserInfo 是网络请求，可能会在 Page.onLoad 之后才返回
              // 所以此处加入 callback 以防止这种情况
              if(!wx.getStorageSync('isAgree')){
                wx.setStorage({
                  key: 'isAgree',
                  data: true,
                })
                // 如果是第一次同意则
                var userInfo = codeData.code + ":" + JSON.stringify(res)
                this.getToken(codeData.code, userInfo);
              }else{
                // 如果同意过一次则走
                this.getToken(codeData.code, codeData.code);
              }
              if (this.userInfoReadyCallback) {
                this.userInfoReadyCallback(res)
              }
  ...
  getToken(code,userInfo) {
    wx.request({
      ...
      // url:'http://10.10.30.37:9010/oauth/token', //仅为示例，并非真实的接口地址
      method: "POST",
      data: {
        grant_type: 'password',
        username: 'WXM_' + this.data.appid + ':' + userInfo ,
        password: code,
      },
      header: {
        "content-type": 'application/x-www-form-urlencoded', // 默认值
