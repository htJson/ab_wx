...
Page({
  ...
  getCount(){
    app.req({ "query": 'query{my_student_exam_score_counts{total,passed,failed}}'}, res => {
      this.setData({
  ...
  getList(){
    ...
    app.req({ "query": 'query{my_student_exam_apply_result{examPlan{exam_plan_id,exam_date},examApply{score},course{name,course_id}}}'}, res => {
      this.setData({
        loading: false,
      })
